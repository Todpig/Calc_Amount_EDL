def get_values(period, type):
    ...
    if type == "yearly":
        ...
        [amount, totalInvested, totalIncome] = calcAmount(valueInit, period * 12,
                                                          monthlyInterestRate, monthlyDeposit)

    else:
        # INTEREST RATE MONTHLY
        monthlyInterestRate = (interestRate / 100)
        [amount, totalInvested, totalIncome] = calcAmount(valueInit, period,
                                                          monthlyInterestRate, monthlyDeposit)

    formattedAmount = f"{amount:.{2}f}"
...
def calcAmount(valueInit, period, interestRate, monthlyDeposit):
    ...
    print("-" * 50)
    print("| {:<4} | {:<15} | {:<18} |".format(
        "Mês", "Montante/mês", "Rendimento/mês"))
    print("-" * 50)

    totalIncome = 0  # Inicializa o total de rendimentos

    for i in range(period):
        totalInvested += monthlyDeposit
        monthlyIncome = amount * interestRate  # Calcula o rendimento mensal
        totalIncome += monthlyIncome  # Acumula o rendimento mensal
        amount = amount * (1 + interestRate)
        amount += monthlyDeposit
        print("| {:<4} | {:<15.2f} | {:<18.2f} |".format(
